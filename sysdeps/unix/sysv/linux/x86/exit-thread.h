/* Call to terminate the current thread.  Linux/x86 version.
...
#if defined __CET__ && (__CET__ & 2) != 0
# ifdef __x86_64__
      unsigned long long ssp_base, ssp_size;
# else
      unsigned long ssp_base, ssp_size;
# endif
      struct pthread *self = THREAD_SELF;
      ssp_base = THREAD_GETMEM (self, header.ssp.base);
      if (ssp_base)
	{
	  /* Free the shadow stack before exit.  */
	  ssp_size = THREAD_GETMEM (self, header.ssp.size);
	  INTERNAL_SYSCALL (munmap, err, 2, ssp_base, ssp_size);
	}
#endif
