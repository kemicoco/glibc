/* Linux/x86 getrlimit implementation (32 bits rlim_t).
...
{
  int res = INLINE_SYSCALL_CALL (ugetrlimit, resource, rlim);
  if (res == 0 && resource == RLIMIT_AS)
    {
      if (rlim->rlim_cur != RLIM_INFINITY)
	rlim->rlim_cur -= GL(dl_x86_legacy_bitmap)[1];
      if (rlim->rlim_max != RLIM_INFINITY)
	rlim->rlim_max -= GL(dl_x86_legacy_bitmap)[1];
    }
  return res;
}
...
{
  int res = INLINE_SYSCALL_CALL (getrlimit, resource, rlim);
  if (res == 0 && resource == RLIMIT_AS)
    {
      if (rlim->rlim_cur != RLIM_INFINITY)
	rlim->rlim_cur -= GL(dl_x86_legacy_bitmap)[1];
