...
{
  /* Check CET status and legacy region bitmap info.  */
  unsigned long cet_status[3];
  INTERNAL_SYSCALL_DECL (err);
  int res = INTERNAL_SYSCALL (arch_prctl, err, 2,
			      ARCH_CET_CONTROL_STATUS, cet_status);
  if (res == 0)
    {
      /* Update dl_x86_feature_1 and dl_x86_legacy_bitmap.  */
      GL(dl_x86_feature_1) = cet_status[0];
      GL(dl_x86_legacy_bitmap)[0] = cet_status[1];
      GL(dl_x86_legacy_bitmap)[1] = cet_status[2];
    }
